 ...
 *
 * Changing the referral code is deliberate either way — typed by hand, or
 * regenerated by ticking reissue — because a code is an identifier that has
 * already been printed on posters and pasted into bios. Changing it silently
 * would break links that are out in the world, so an untouched field is
 * treated as "leave it alone" rather than as a write.
 *
 * Reissue wins over a typed code: the box is disabled while it is ticked, so
 * anything still in it is what the field held before, not an instruction.
 */
export async function updateAmbassadorSegments(
  ...
  try {
    ...
    // Needed to tell "left the code alone" from "typed the same thing back",
    // so an untouched field never counts as a change worth auditing.
    const { data: current } = await supabase
      ...
      .maybeSingle();
    if (!current) {
      return { ok: false, message: "That ambassador no longer exists." };
    }

    let newCode: string | null = null;

    if (reissue) {
      newCode = await nextReferralCode(createAdminClient(), batch);
    } else if (typedCode && typedCode !== current.referral_code) {
      // Re-checked here and not only in the dialog: the action is reachable
      // without it, and a code that fails these rules is a dead share link.
      const problem = codeProblem(typedCode);
      if (problem) return { ok: false, message: problem };

      // Named rather than left to the unique constraint. "duplicate key value
      // violates profiles_referral_code_key" is true and useless; an admin
      // needs to know somebody already has it.
      const { data: clash } = await createAdminClient()
        .from("profiles")
        .select("id, full_name")
      ...
      if (clash && clash.id !== profileId) {
        return {
          ok: false,
          message: `${typedCode} already belongs to ${clash.full_name || "another ambassador"}.`,
        };
      }
    ...
    const { error } = await supabase
      ...
      .eq("id", profileId);
    if (error) {
      // The check above can lose a race with another admin saving the same
      // code. The constraint is what actually guarantees uniqueness, so its
      // rejection gets the same readable message rather than a raw one.
      if (error.code === "23505") {
        return {
          ok: false,
          message: `${newCode} was taken while you were editing. Try another.`,
        };
      }
      throw error;
    }
    ...
    await audit(actorId, "ambassador.segments", "profile", profileId, {
      ...
      // Kept so a code can be traced back after the fact. A hand-typed one is
      // not reconstructable from the batch the way a generated one is.
