...
/**
 * Redemption requests and payout batches.
 *
 * Lives inside the Stipend section rather than on a page of its own. Both are
 * the same job — money leaving the programme — and splitting them across two
 * destinations meant deciding a redemption and paying a stipend batch were two
 * clicks apart for no reason.
 */
export async function PayoutQueue() {
  const [redemptions, batches] = await Promise.all([
    getRedemptions(),
    getPayoutBatches(),
  ]);
      ...
      <Card>
        <CardBody>
          ...
          {approvedWaiting.length > 0 && (
            <Note
              tone="ok"
              title={`${approvedWaiting.length} approved and waiting`}
              className="mt-4"
            >
      ...
      {batches.length === 0 ? (
        <Card>
          <EmptyState
            ...
            description="Build one from eligible stipends above, or from approved redemptions."
          />
        </Card>
      ) : (
        batches.map((batch) => (
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="ml-auto flex gap-2">
                  <Button size="sm" variant="secondary" asChild>
                    <a href={`/admin/stipend/batch/${batch.id}/export`}>
